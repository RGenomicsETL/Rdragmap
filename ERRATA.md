...
`meta/patches/0006-darwin-hash-semaphore.patch`. The imported
static-library recipes also requested GNU `ar`’s filename-truncation
option, which Darwin `ar` rejects. They now use the portable create,
replace, and index options recorded in
`meta/patches/0007-portable-static-archive-options.patch`. These changes
affect only build and platform-support code, not reference indexes,
mapping decisions, or SAM records.
...
### E-016: Memory-mapped index file extents

The imported memory-mapping deleter used the hash-table byte count for
every mapped file. On the package’s 16 MiB tiny index, `reference.bin`
is 164,352 bytes; teardown therefore called `munmap()` for 16 MiB at the
reference mapping, unmapped adjacent shared libraries, and terminated
with `SIGSEGV` after already writing SAM and metrics. Rdragmap captures
each file’s validated extent in its own deleter. Open descriptors retain
their imported process-lifetime behavior; this correction changes only
the erroneous unmap length. It does not affect index bytes or mapping
decisions and is recorded in
`meta/patches/0008-mmap-file-extents.patch`.

The package tinytest builds both uncompressed files, maps them during a
real alignment, and requires the child process to return normally. It
also rejects an incomplete mmap request before launching the native
process. Both clean native builds and the tarball package check compile
and execute the regression path.
