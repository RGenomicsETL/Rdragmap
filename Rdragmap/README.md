...
`mmap_reference = TRUE` to `rdragmap_align()` maps those files directly.
This avoids per-process hash decompression and allows the operating
system to share cached index pages between processes. The tradeoff is
disk space. For the executed hs37d5 index, the two additional files
occupy about 30 GiB, compared with a 4.0 GiB compressed hash table.
...
[`benchmarks/hs37d5-hg02088`](../benchmarks/hs37d5-hg02088/README.md)
record.
